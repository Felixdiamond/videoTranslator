...
def create_synchronized_translation(video_path, transcription_result, translated_text, target_language):
    ...
    # Detect sound events in the original audio
    original_audio_path = tempfile.mktemp(suffix=".wav")
    video.audio.write_audiofile(original_audio_path)
    sound_events = detect_sound_events(original_audio_path)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        for i, (phrase, translated_sentence) in enumerate(matched_pairs):
            ...
            try:
                ...
                try:
                    ...
                    print(f"Error reading WAV file {temp_path}: {str(e)}")
                    print(f"File size: {os.path.getsize(temp_path)} bytes")
                    with open(temp_path, 'rb') as f:
                        print(f"First 20 bytes: {f.read(20)}")
                    continue  # Skip this phrase if there's an error
                ...
                fade_duration = min(50, len(phrase_audio) // 2)
        ...
        # Add sound events from the original audio
        for event_time in sound_events:
            event_audio = AudioSegment.from_wav(original_audio_path)
            event_duration = 100  # ms
            event_audio = event_audio[int(event_time * 1000):int(event_time * 1000) + event_duration]
            final_audio = final_audio.overlay(event_audio, position=int(event_time * 1000))

    temp_audio_path = tempfile.mktemp(suffix=".wav")
    final_audio.export(temp_audio_path, format="wav")
    
    final_audio_clip = AudioFileClip(temp_audio_path)
    final_audio_clip = final_audio_clip.set_duration(video.duration)
